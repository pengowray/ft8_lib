...
const annotationDefinitions = {
    ...
    "0.1": [
        { label: "Call A", tag:'c28a', start: 0, length: 28, getValue: bitsToCall },
        { label: "Call B", tag:'c28b', start: 28, length: 28, getValue: bitsToCall },
    ...
    "1": [ // Standard message
        { label: "Call A", tag:'c28a', start: 0, length: 28, getValue: bitsToCall },
        { label: "/R A", shortLabel:'r', tag:'r1a', start: 28, length: 1, getValue: (bit) => bit === '1' ? '1 /R' : '0' },
        { label: "Call B", tag:'c28b', start: 29, length: 28, getValue: bitsToCall },
        { label: "/R B", shortLabel:'r',  tag:'r1b', start: 57, length: 1, getValue: (bit) => bit === '1' ? '1 /R' : '0' },
    ...
    "2": [ // EU VHF Contest c28 p1 c28 p1 R1 g15
        { label: "Call A", tag:'c28a', start: 0, length: 28, getValue: (bits) => bitsToCall(bits) },
        { label: "/P A", shortLabel:'p', tag:'p1a', start: 28, length: 1, getValue: (bit) => bit === '1' ? '/P' : '' },
        { label: "Call B", tag:'c28b', start: 29, length: 28, getValue: (bits) => bitsToCall(bits) },
        { label: "/P B", shortLabel:'p', tag:'p1b', start: 57, length: 1, getValue: (bit) => bit === '1' ? '/P' : '' },
    ...
    "3": [ // ARRL RTTY Roundup t1 c28 c28 R1 r3 s13
        ...
        { label: "Call A", tag:'c28a', start: 1, length: 28, getValue: (bits) => bitsToCall(bits) },
        { label: "Call B", tag:'c28b', start: 29, length: 28, getValue: (bits) => bitsToCall(bits) },
    ...
    "4": [ // Non-standard call: h12 c58 h1 r2 c1
        ...
        { label: "Order", shortLabel: "h", tag: 'h1', start: 70, length: 1, getValue: (bit) => bit === '1' ? '1 (Hash is second callsign)' : '0 (Hash is first callsign)' },
    ...
    "5": [ // EU VHF Contest with 6-digit grid locator
        { label: "Hash A", tag:'h12a', start: 0, length: 12, getValue: bitsToHash },
        { label: "Hash B", tag:'h22b', start: 12, length: 22, getValue: bitsToHash },
